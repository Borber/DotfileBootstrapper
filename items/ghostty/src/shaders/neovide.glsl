// Adapted from https://codeberg.org/JellyCat/JellyDotFiles/src/branch/main/items/ghostty/src/shaders/neovide.glsl
...
float getSdfConvexHex(in vec2 p, in vec2 v1, in vec2 v2, in vec2 v3, in vec2 v4, in vec2 v5, in vec2 v6) {
    float s = 1.0;
    float d = dot(p - v1, p - v1);
...
float getProgress(float elapsed, float duration) {
    if (duration <= 0.0) {
        return 1.0;
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION - 0.001) {
        ...
        float prog_tl = getProgress(baseProgress, final_dur_tl);
        float prog_tr = getProgress(baseProgress, final_dur_tr);
        float prog_bl = getProgress(baseProgress, final_dur_bl);
        float prog_br = getProgress(baseProgress, final_dur_br);

        float diagSideDuration = (1.0 - SHAPE_FACTOR) * DURATION_LEAD + SHAPE_FACTOR * DURATION_TRAIL;
        float prog_side = getProgress(baseProgress, diagSideDuration);

        vec2 side_tl = mix(cp_tl, cc_tl, prog_side);
        vec2 side_tr = mix(cp_tr, cc_tr, prog_side);
        vec2 side_bl = mix(cp_bl, cc_bl, prog_side);
        vec2 side_br = mix(cp_br, cc_br, prog_side);
        ...
        float sdfTrailQuad = getSdfConvexQuad(vu, v_tl, v_tr, v_br, v_bl);

        float sdfTrailHex = sdfTrailQuad;
        if (isDiagonal > 0.5) {
            if (s.x < 0.0 && s.y < 0.0) {
                sdfTrailHex = getSdfConvexHex(vu, cc_tl, cc_bl, cc_br, side_br, v_tr, side_tl);
            } else if (s.x > 0.0 && s.y < 0.0) {
                sdfTrailHex = getSdfConvexHex(vu, side_tr, cc_tr, cc_br, cc_bl, side_bl, v_tl);
            } else if (s.x < 0.0 && s.y > 0.0) {
                sdfTrailHex = getSdfConvexHex(vu, cc_tl, cc_tr, side_tr, v_br, side_bl, cc_bl);
            } else if (s.x > 0.0 && s.y > 0.0) {
                sdfTrailHex = getSdfConvexHex(vu, cc_tl, cc_tr, cc_br, side_br, v_bl, side_tl);
            }
        }

        float sdfTrail = mix(sdfTrailQuad, sdfTrailHex, isDiagonal);
        ...
        if (BLUR < 2.5) {
          effectiveBlur = mix(0.001, BLUR, isDiagonal);
        }
